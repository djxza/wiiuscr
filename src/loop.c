...
void draw_img(u2 pos, tex_t *pt, screen_t *ps) {
  ...
  // Copy row by row (more efficient than pixel by pixel)
  for (u32 y = start_y, src_y = src_start_y; y < end_y; ++y, ++src_y) {
    // Calculate row indices
    usize src_row_start = src_y * pt->size.width + src_start_x;
    usize dst_row_start = y * ps->properties.width + start_x;
    u32 copy_width = end_x - start_x;

    // Copy entire row at once
    memcpy(&ps->pixels[dst_row_start], &pt->pixels[src_row_start],
...
void draw_tile(int x, int y, screen_t *ps) { //
  draw_img((u2){x, y}, &tile, ps);
}
...
void draw_tile_shadow(u2 pos, s2 size, screen_t *ps) {
  ...
  for (int y = 0; y < size.height; ++y) {
    for (int x = 0; x < size.width; ++x) {
      ...
      u8 a = 40; // soft shadow
      put_pixel((u2){px, py}, rgba_to_u32(0, 0, 0, a), ps);
    }
  }
}
